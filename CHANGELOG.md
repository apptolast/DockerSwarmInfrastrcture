...
- Primera activación de la actualización automática: `kropia`,
  ...
  mala se deshace con el rollback de Swarm (con varios minutos sin servicio
  por intento). `minecraft`, `passbolt`, `shlink` y `organizationweb` siguen
  en `false` mientras no exista backup fuera del host (STOP gate 5);
  `openclaw` y `n8n` siguen en hold de versión, y Traefik queda fuera por su
  radio de impacto.
- El runbook de `docs/AUTOUPDATE.md` admite que, tras un apply que cambió
  specs, la primera repetición informe `changed=1` sin reiniciar tareas; la
  segunda debe dar `changed=0`. Se observó así en `edge`, `workloads` y
  `organizationweb` el 2026-09-13, con causa sin identificar. Se corrige
  además que `docker_stack` compare el inspect completo: solo compara `Spec`.
...
- Personalización OrganizationWeb21 `dfac90e` desplegada y aceptada con
  ...
  151 E2E; apply38/5/0 EXIT0 y operación liberada. API/web healthy con
  digests revisados; PG/Rabbit y datos previos preservados, Flyway20 válida.
  Lecturas HTTPS/defaults, cinco observaciones Chromium, ocho rutas legacy
  y logout verificados, sin consumir plazas con campos QA. Evidencia y
  límites de mutación/rollback en `docs/ORGANIZATIONWEB.md`.
...
- Apariencia de OrganizationWeb20 desplegada y aceptada con catálogo
  ...
  38/5/0, ambos EXIT 0. Backend y web confirmaron actualización `completed`.
  Aceptación HTTPS de preferencia guardada/recargada, datos anteriores y
  rutas legacy documentada en `docs/ORGANIZATIONWEB.md`. Sólo cambiaron
  release y digests API/web. Se conservan el backup fresco restaurado y el
  ensayo local de retroceso V19, sin ampliar sus límites a rollback Swarm,
  escrituras posteriores o respaldo externo programado.
...
- Traefik se reiniciaba en bucle y cortaba todo el tráfico público. Con un
  ...
  tareas en 24 h el 2026-09-25). El límite pasa a 256 MiB. Como `edge` no se
  puede aplicar (compuerta STOP 10), el mismo valor se aplicó en vivo el
  2026-09-25 con `docker service update --limit-memory 256M`, sin tocar nada
  más del servicio.
- `portfolio-alberto` pasa de 128 a 256 MiB de límite: `next-server` chocaba
  con él (un OOM del cgroup y dos reinicios por healthcheck entre el
  2026-09-15 y el 2026-09-22). Ambos aumentos salen del presupuesto que
  liberan Minecraft y OpenClaw aparcados; desaparcarlos exige ahora 256 MiB
  más de decisión de capacidad (`docs/CAPACITY.md`).
...
- La imagen de runners de n8n vuelve a `n8nio/runners:2.31.5`, la misma
  ...
  `n8nio/runners`, fijada por digest y con instrucciones `FROM` planas, sin
  variables ni directivas; que la pareja formada por el hold de n8n que se
  renderiza (`config/image-channels.yml`) y esa base, con tags y digests, sea
  una pareja revisada; y que la etiqueta de la etapa final nombre la misma
...
- El vigilante espera una hora antes de reiniciarse (`restart_policy.delay`
  ...
  quedaba casi siempre caído y gastaba el cupo de Docker Hub. Es una
  mitigación: con la cabeza rota sigue habiendo un intento por hora, y los
  servicios siguientes del ciclo quedan sin revisar. Un apply idéntico de
  `autoupdater` falla mientras el reinicio está pendiente. El validador fija
  el nuevo valor y un test rechaza el anterior.
